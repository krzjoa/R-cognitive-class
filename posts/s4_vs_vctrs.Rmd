...
bm1 <- microbenchmark(
  #s3_plain = pilsner + my_opener,
  s4 = S4_beer + S4_opener,
  s3_vec = vec_pilsner + my_vec_opener,
  times = 1000
)

#bm$ntime <- microbenchmark:::convert_to_unit(bm$time, "t")

# ggplot(bm) +
#   geom_beeswarm(aes(x=expr, y=ntime))
...
bm2 <- microbenchmark(
  #s3_plain = pilsner + my_opener,
  s4 = S4_opener + S4_beer,
  s3_vec = my_vec_opener + vec_pilsner,
  times = 1000
)
